...
## ============================================================
## 掉落物实体 (ItemDrop) — 可拾取的旋转悬浮小方块
## ============================================================
## 职责：
##   1. 以缩小的3D立方体展示方块纹理（复用体素Shader材质）
##   2. 物理模拟：重力下落 + 地面摩擦减速
##   3. 落地悬浮：上下正弦抖动 + Y轴旋转（仿Minecraft效果）
##   4. 吸附拾取：进入玩家磁铁范围后飞向玩家，接近后触发拾取
##   5. 堆叠拾取：玩家快捷栏优先合并已有堆叠，否则填入空位
##   6. 丢弃保护：Q丢弃后有短暂免吸附时间（attract_delay）
##
## 状态机：
##   ready → 下落（有重力）→ 落地（悬浮旋转）→ [玩家进入范围] → 飞向玩家 → 拾取/消失
## ============================================================
